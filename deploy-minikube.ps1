# ══════════════════════════════════════════════════════════════════════════════
# InternConnect — Full Kubernetes Deployment Script for Windows (PowerShell)
# Usage: Right-click → Run with PowerShell  OR  .\deploy-minikube.ps1
# Requirements: Docker Desktop running, C:\minikube\minikube.exe installed
# ══════════════════════════════════════════════════════════════════════════════
...
if (-Not (Test-Path $MINIKUBE)) {
    ...
    Invoke-WebRequest -OutFile $MINIKUBE `
        -Uri 'https://github.com/kubernetes/minikube/releases/latest/download/minikube-windows-amd64.exe' `
        -UseBasicParsing
    Write-Info "minikube installed at $MINIKUBE ✓"
} else {
    Write-Info "minikube found at $MINIKUBE ✓"
}
...
Write-Info "Backend image built ✓"
...
Write-Info "Frontend image built ✓"
...
Write-Info "Namespace 'internconnect' created ✓"
...
Write-Info "Frontend and Nginx ready ✓"
...
Write-Info "CPU and Memory limits are defined in all deployment YAMLs ✓"
...
if (-Not $helmInstalled) {
    ...
    Write-Info "Helm installed ✓"
} else {
    Write-Info "Helm found ✓"
}
...
Write-Host ""
Write-Info "🚀 Open App in Browser:"
& $MINIKUBE service nginx-service -n internconnect

Write-Host ""
Write-Info "📊 Access Grafana (run in a new terminal):"
...
Write-Host ""
Write-Info "📊 Access Prometheus (run in a new terminal):"
